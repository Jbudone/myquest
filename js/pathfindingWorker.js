...
// TODO
//  - Compilation/fuckingtaskrunner (right now going right through source), reference from dist/js rather than js
...
const handlePath = (path) => {
    ...
    // FIXME: Assume we only have from/to tiles and want tiled A*


    const fromTiles = [path.startTile],
        toTiles = [path.destinationTile];

    let now = Date.now();
    const revisedPath = findPathTILE(fromTiles, toTiles, 100);
    let tileTime = (Date.now() - now);

    now = Date.now();
    const ptPath        = findPathPOINT(path.startPt, path.endPt);
    let ptTime = (Date.now() - now);

    if (revisedPath) {

        // We may have no path (from/to are the same)
        if (!revisedPath.path) {
            path.ALREADY_THERE = true;
        } else {
            path.walks = revisedPath.path.walks;
            path.start = revisedPath.path.start;
        }
        ...
        if (ptPath) {
            if (!ptPath.path) {
                ...
                path.ptPath = {
                    start: ptPath.path.start,
                    walks: ptPath.path.walks
                };
            }
        }

        path.tileTime = tileTime;
        path.ptTime = ptTime;
...
const localFromGlobalCoordinates = (x, y) => {
...
const isTileOpen = (tile) => {
    const localCoordinates = localFromGlobalCoordinates(tile.x, tile.y);

    if (!localCoordinates.page) return true; // We don't have this page, so lets just assume its open?
...
const findPathPOINT = (fromPt, toPt, options) => {
    // FIXME
    ...
    // FIXME: If we just use tiles then recalibrate we can still hit other tiles. Need to do 1x1 tile search
    // for now until we can upgrade pathfinding w/ better optimizations
...
const findPathTILE = (fromTiles, toTiles, _maxWeight) => {
    ...
    if (nearestEnd) {
        ...
        path.walks.unshift(walk);
        walk.destination = nearestEnd.tile;
